Kernel_hardening_via_sysctl(){
	print_info "Applying sysctl hardening..."
	[ -f "/etc/sysctl.d/99-hardening.conf" ] && return
    ...
    if [[ $(</proc/sys/kernel/yama/ptrace_scope) != 1 ]]; then
    	print_success "ptrace is restricted."
	fi
	print_success "Kernel_hardening_via_sysctl completed..."
}
functions_list="$functions_list Kernel_hardening_via_sysctl"
