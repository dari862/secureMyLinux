Blacklisting_unused_kernel_modules(){
	[ -f "/etc/modprobe.d/99-custom-blacklist.conf" ] && return
	print_info "Blacklisting unused kernel modules"

	tee /etc/modprobe.d/99-custom-blacklist.conf > /dev/null <<-EOF
	...
	# Framebuffer drivers (Ubuntu source)
	...
	print_success "Kernel modules blacklisted in: $BLACKLIST_FILE"
}
functions_list="$functions_list Blacklisting_unused_kernel_modules"
